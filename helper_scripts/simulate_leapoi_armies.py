...
def run_simulation(factions: str = "all", difficulties: str = DIFFICULTIES, count: int = 1, seed: int = 1, mods: str = "none") -> SimResult:
    ...
    args = [lua, SIM_SCRIPT, os.path.abspath(LEAPOI_SOURCE_ROOT), f"faction={factions}", f"difficulty={difficulties}", f"count={count}", f"seed={seed}", f"mods={mods}"]
    completed = subprocess.run(args, capture_output=True, text=True, encoding="utf-8")
    if completed.returncode != 0:
        raise RuntimeError(f"The army simulator failed: {completed.stderr.strip()}")
