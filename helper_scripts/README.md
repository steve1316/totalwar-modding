...
- **Publishing** (`workshop_publish.py` + `workshop_publisher/publish.js`): after building, every generated pack that differs from what was last uploaded is listed with an auto-generated change note (e.g. "Updated for changes in: Mixu's Legendary Lords."). The TTC compat note instead lists the units added since the last upload by in-game name under their faction (vanilla and DLC units in their own section, modded units grouped by mod within each faction). When it would pass Steam's 8000-character limit it drops to mod counts per faction, then to faction counts only, with removed unit keys always at the bottom. A modded unit's faction comes from the race code in its key (`emp`, `skv`, ...), or else from the recruit groups that can recruit it, and is `Other` when neither names one. After a preflight confirms each item exists and is owned by the logged-in Steam account, `update.py` asks `Publish N item(s) to the Steam Workshop? [y/N]`. On `y` it uploads the pack and its preview png as the item's content with that change note, leaving the title, description, tags and visibility untouched, then prints each Workshop URL with its result. Items that were not published stay pending for the next run. It never uploads outside an interactive terminal.
